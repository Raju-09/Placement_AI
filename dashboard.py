...
<style>
    ...
    /* Strict override for radio label colors (ensure high contrast labels) */
...
if page == "🎯 Prediction":
    ...
    with col2:
        with st.container(border=True):
            ...
            # Custom styled HTML Predictions Table (Programmatically Dedented)
    ...
    # Render customized HTML cards for recommendations (Programmatically Dedented)
    recommendations_html = "<div class='rec-container'>"
    for icon, title, desc in recommendations:
        recommendations_html += textwrap.dedent(f"""
        <div class="rec-item">
